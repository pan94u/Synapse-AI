...
## Session 14 — Skill 发布体验升级：发布向导 + SKILL.md 导入 + 生命周期可视化 + 数据修复

**日期**: 2026-03-05
**Commit**: `4e01206` feat: Skill 发布向导 + SKILL.md 导入 + 生命周期可视化

### Plan（目标）

让用户在页面上就能完成「创建/导入 → 预览评分 → 填写元数据 → 发布 → 查看结果」的完整闭环，消除"必须 curl 调 API 才能发布"的痛点。

### Do（实施）

**1. 后端 — 新增 2 个 API**

- `POST /api/skills/import` — 接收 SKILL.md 原文，调用 `parseSkillMd()` 解析 frontmatter + body，提取 name/description/category/allowedTools/parameters/metadata/instructions，调用 `createCustomSkill()` 创建技能
- `POST /api/marketplace/preview` — 接收 skillId，调用 `publishReview()` 返回审核评分预览（不实际发布）
- `createMarketplaceRoutes()` 签名增加可选 `skillManager` 参数，`app.ts` 传入

**2. 前端 — 3 步发布向导 (publish-wizard.tsx)**

新建 `publish-wizard.tsx`，Dialog 形式的 3 步向导：
- **Step 1 选择来源**: 两个卡片（选择已有 Skill / 导入 SKILL.md），Select 下拉 + Textarea 粘贴
- **Step 2 审核预览**: 调用 preview API 显示总分（绿≥70/黄30-69/红<30）+ 4 维度检查卡片 + 预测结果
- **Step 3 元数据发布**: 作者名称/版本号/标签表单 → 调用 publish API → inline 展示成功/失败

**3. 前端 — 生命周期可视化 + 快速发布入口**

`submission-guide.tsx` 顶部新增两个 Card：
- **Skill 生命周期流程图**: 3 行 Badge + 箭头，展示 publish → active/pending → suspended/rejected/deprecated 的 5 态流转
- **快速发布入口**: 大按钮 "立即发布 Skill →" 打开 PublishWizard

**4. 布局 & 数据修复（第 2-3 轮迭代）**

- **Stats API 路径修复**: `MarketplaceStats` 组件调用 `/marketplace/stats` → 改为 `/marketplace/status`（与后端路由匹配）
- **Stats 字段映射修复**: 前端 `Stats` 接口字段名与后端 `MarketplaceStats` 类型对齐（`totalPublished`/`totalInstalled`/`totalReviews`/`categoryCounts`）
- **Tab 顺序调整**: 默认 tab 从 "浏览" 改为 "接入指南"，tab 顺序: 接入指南 → 浏览 → 已安装 → 审核队列
- **发布按钮布局**: 从独立行右浮移到 TabsList 同行右侧
- **内置技能预上架**: `app.ts` 启动时遍历 13 个 active built-in skills，自动 `marketplace.publish()` 上架（已存在则跳过）
- **MarketplaceSkill 类型修复**: `skill-browser.tsx` 中 `author` 从 `string` 改为 `{ id, name }` 对象，`rating` 从 `number` 改为 `{ average, count }` 对象
- **渲染修复**: `marketplace-card.tsx` 和 `skill-detail-dialog.tsx` 中 `skill.rating.toFixed()` → `skill.rating.average.toFixed()`，`skill.author` → `skill.author.name`
- **API 路径修复**: 安装 `/marketplace/${id}/install` → `/marketplace/skills/${id}/install`，评价路径同理

**5. i18n**

`zh.ts` 新增 `marketplace.publish`（向导全部文案）、`marketplace.lifecycle`（生命周期状态名 + 转换说明）、`marketplace.quickPublish`（快速发布卡片文案），更新 `marketplace.stats` 标签、`workflow.steps[2]` 描述

### Check（验证）

| 测试项 | 结果 |
|--------|------|
| `tsc --noEmit` (server + web) | 2/2 通过 |
| Docker 重建 (synapse-server + synapse-web) | 成功 |
| 服务器启动日志 | `Auto-seeded 13 built-in skills to marketplace` |
| `http://localhost:19300/marketplace` | 默认显示接入指南 tab |
| 接入指南 tab | 生命周期流程图 + "立即发布 Skill" 按钮可见 |
| 浏览 tab | 13 个 Skill 卡片正常渲染（author/rating/downloads） |
| Stats 区域 | 5 个统计卡片正常显示（已发布 13/已安装 0/待审核 0 等） |
| 发布向导 Step 1→2→3 流程 | 可走通 |

### Act（经验沉淀）

- **前后端类型不一致是头号 Bug 源**: `MarketplaceSkill` 前端接口与后端实际返回结构不匹配（author 对象 vs 字符串、rating 对象 vs 数字），导致 `.toFixed()` 运行时崩溃。教训：前端类型定义必须以后端 `@synapse/shared` 中的类型为准
- **API 路径命名一致性**: Stats 端点后端叫 `/status`，前端调的是 `/stats`，静默失败。路由命名必须前后端逐一对照
- **空数据 = 无用页面**: 浏览 tab 空空如也对用户毫无价值，预上架内置技能是必要的 Day-1 体验
- **Tab 默认值**: 市场页面默认打开"浏览"（空的）不如默认打开"接入指南"（有内容），新功能应以"引导"为默认视角

### 统计快照

| 指标 | 值 |
|------|-----|
| 新建文件 | 1 (publish-wizard.tsx) |
| 修改文件 | 10 (skills.ts, marketplace.ts, app.ts, submission-guide, marketplace-panel, marketplace-stats, marketplace-card, skill-browser, skill-detail-dialog, zh.ts) |
| 代码行数 (净增) | ~+750 |
| 新增 API 端点 | 2 (POST /api/skills/import, POST /api/marketplace/preview) |
| Bug 修复 | 6 (stats API 路径, stats 字段映射, MarketplaceSkill 类型, rating/author 渲染, install/review API 路径, tab 默认值) |
| Commits | 2 (`4e01206`, `c905ba6`) |
| 类型检查 | 2/2 通过 |

---

## Session 15 — Feishu MCP Server 真实 API 接入 + Persona 日期感知

**日期**: 2026-03-06
**Commits**: `2736449` feat: Feishu MCP Server 真实 API 接入 + CEO 角色飞书集成
            `1696c49` feat: Persona system prompt 注入当前日期
            `b66b552` chore: 文档更新 + 运行时数据归档

### Plan（目标）

将飞书 MCP Server 从纯 Mock 升级为真实 API + Mock 降级模式，打通 CEO Persona 的飞书操作能力；同时为所有 Persona 的 system prompt 注入当前日期，消除 Agent 的时间盲点。

### Do（实施）

1. **Feishu MCP Server 真实 API 接入** (`packages/mcp-servers/src/feishu/index.ts`, +345 行)
   - `fetchTenantToken()` + `getToken()`: 自动获取并缓存 `tenant_access_token`（7200s TTL，提前 5min 刷新），避免每次请求都鉴权
   - `feishuGet()` / `feishuPost()`: 统一封装 HTTP 请求，自动注入 Bearer Token，401 时自动刷新一次 token 再重试
   - **8 个 MCP 工具实现**:
     - `send_message`: 发飞书消息（支持 text / interactive 卡片）
     - `list_messages`: 查询群聊消息历史（含发送人 open_id + 时间戳）
     - `list_calendar`: 获取日历事件列表（时间段过滤）
     - `create_event`: 创建日历事件（支持 attendees 批量邀请）
     - `create_approval`: 创建审批单（支持多种审批表单字段）
     - `get_approval`: 查询审批单状态
     - `read_doc`: 读取飞书文档内容
     - `search_docs`: 全文搜索飞书文档
   - **Mock 降级**: `FEISHU_APP_ID` / `FEISHU_APP_SECRET` 未配置时自动切换 Mock 数据，启动时打印告警

2. **Feishu MCP Server 配置更新** (`config/mcp-servers/feishu.json`)
   - `enabled: true` / `autoStart: true`（原为 false）
   - `env` 透传 `FEISHU_APP_ID` / `FEISHU_APP_SECRET` 环境变量（`${env:VAR}` 格式）
   - 标签增加 `docs`，requireApproval 使用短工具名对齐新实现

3. **CEO Persona 飞书接入** (`config/personas/ceo.yaml`)
   - `allowed_mcp_servers` 增加 `feishu`
   - `allowed_tools` 授权 8 个 `feishu_*` 工具
   - `description` 注入飞书环境上下文：团队群 `chat_id`、主日历 `calendar_id`、3 位成员 `open_id`，让 Agent 无需追问即可直接操作

4. **Persona system prompt 日期注入** (`packages/personas/src/context.ts`)
   - `buildSystemPrompt()` 在【交互风格】之前插入 `【当前时间】YYYY年M月D日 星期X`
   - 用 `new Date()` 实时生成，无需 LLM 工具调用即可感知当前日期

### Check（验证）

| 测试项 | 结果 |
|--------|------|
| `tsc --noEmit` (mcp-servers, personas) | 通过 |
| Mock 模式启动（无凭证） | `[feishu] FEISHU_APP_ID / FEISHU_APP_SECRET 未配置，使用 Mock 模式` |
| CEO Persona 工具列表 | feishu_* 8 个工具可见 |
| System prompt 日期显示 | `【当前时间】2026年3月6日 星期五` |

### Act（经验沉淀）

- **Token 缓存 + 提前刷新**: 飞书 `tenant_access_token` 有效期 7200s，缓存 + 提前 300s 刷新可兜住边界情况；401 时原地重试一次是必要的保险
- **环境上下文注入 description**: CEO description 里硬编码 `chat_id` / `calendar_id` / `open_id`，比在每次对话中询问用户效率高得多；这是 Persona 配置的一个通用模式
- **日期注入 vs 工具调用**: 将当前日期直接写入 system prompt，比给 Agent 一个 `get_current_date` 工具更简洁，减少一次无意义的 tool call round-trip
- **Mock 降级设计**: 条件式 Mock 让开发者在无真实凭证时仍可跑通全流程，生产环境配置环境变量即自动切换，零代码改动

### 文件变更表

**修改 3 个文件**:
```
packages/mcp-servers/src/feishu/index.ts  # 重写: Mock → 真实 API + Mock 降级（515 行，+345）
config/mcp-servers/feishu.json            # enabled/autoStart + env 变量透传
config/personas/ceo.yaml                  # feishu MCP 接入 + 8 工具授权 + 环境上下文
packages/personas/src/context.ts          # buildSystemPrompt 注入当前日期（+6 行）
```
...
| 新建文件 | 0 |
| 修改文件 | 4 |
| 代码行数 (净增) | +351 (feishu +345, context.ts +6) |
